...
- **간단 ⚡ / 정밀 🔬 분석 모드** — PDF 드롭 시 선택. 간단(약 3~4분/20p)은 배경·문제·방법론·수식 4개 섹션만 생성하고 시각화·WebSearch를 생략해 구독 사용량을 크게 아낌. 정밀은 전체 7개 섹션 + 인터랙티브 시각화. 간단 기록은 언제든 **[정밀 분석으로 업그레이드]**(분석 시각 보존) 또는 섹션별 개별 생성으로 채울 수 있음. 다이얼로그에 모드별 예상 시간(자가학습 ETA) 표시
- **그림 크롭 실측 파이프라인** — 그림 해설의 이미지를 텍스트 추론이 아니라 실측으로 크롭: 임베디드 이미지 좌표(`pdftohtml -xml`) → 잉크 밀도(저해상 PGM) → 캡션 텍스트 스캔(표 전용) 라우팅 + 백지·절단 자동 검증([lib/figurebox.js](lib/figurebox.js)). 회귀 하니스: `node scripts/figure_crop_report.js`
- **7개 섹션 분석** — 세미나 정리 / 연구 배경 / 해결하려는 것 / 연구 방법론 / 실험·결과 / 수식 정리 / 그림 해설. 핵심 용어 볼드, 결정적 문장 형광펜, 인라인 `$수식$` KaTeX 렌더링. 제목 아래에 **핵심 기여 2~4줄** 요약
- **세미나 정리 탭** — 논문의 **실제 목차(섹션) 구조를 그대로** 따라 섹션별 핵심을 소번호·페이지 인용과 함께 발표용으로 정리(첫 탭). **오직 논문 내용만** 옮기되, 논문에 Conclusion/Limitation/Future Work가 없을 때만 논문 근거로 보완하고 그 항목은 `추가`로 명시. 페이지 칩(p.N) 클릭 시 원문으로 점프
- **실험·결과 탭** — 측정 지표·데이터셋·용어 설명 + 논문 Experiments를 번호별 실험(목적·세팅·결과)으로 정리. 실험 번호 클릭 시 원문 그 위치로 점프 + ✓
- **그림 해설 탭** — 논문의 Figure·Table을 PDF에서 **실제 이미지로 잘라(poppler)** 보여주고, 원문(영어) 캡션을 번역한 뒤 "무엇이고 어떻게 해석하는지" 해설. 그림 클릭 시 원문 그 그림 위치로 점프 + ✓
- **원문 PDF 나란히 보기** — 좌측 PDF 뷰어 + 우측 분석 결과 (접기 가능)
- **읽기 테마 · 검색 · 딥링크** — 🌗 기본/세피아/다크 테마 전환(저장), 히스토리 제목·요약 검색, `#p=<hash>&tab=` URL로 논문·탭 북마크. 보던 논문을 재분석할 땐 화면을 비우지 않고 상단 배너로 진행만 표시
...
- **실시간 진행 표시 & 취소** — 분석 중 "1-15페이지 읽는 중… / 해설 자료 검색 중…" 단계가 실시간 표시(SSE). **분석 취소** 버튼으로 멈추면 서버의 모델 실행도 함께 중단해 구독 사용량을 아낍니다. 토큰이 만료되면 `claude setup-token` 재실행 안내를 명확히 표시
...
| 메서드 | 경로 | 설명 |
|---|---|---|
| `POST` | `/api/analyze` | PDF 업로드(`pdf` 필드, `mode`=`simple`\|`full`) → SSE 스트림 (progress → result) |
| `POST` | `/api/reanalyze/:hash` | 저장된 원문으로 재분석(body `{mode}`) → SSE 스트림. 간단→정밀 업그레이드도 이 경로 |
| `GET` | `/api/eta?pages=N` | 모드별 예상 소요(ms) — 업로드 다이얼로그의 시간 표시용 |
