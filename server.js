...
스키마:
{
  ...
  "experiments": {
    ...
    "studies": [
      { "title": "실험 이름 (논문 표현 그대로, 예: 'Experiment 1: ...' 또는 '4.2 Ablation')", "purpose": "이 실험으로 무엇을 확인하려는지", "setup": "데이터셋·모델·비교군·조건 등 실험 세팅", "result": "핵심 결과(논문이 보고한 ==실제 수치 포함==)와 그것이 의미하는 바" }
    ],
...
- experiments: 실험·결과 섹션 (전용 탭). ==논문의 Experiments(실험) 절을 보고, 실험을 논문에 나온 번호·순서대로 정리==하세요. 구성:
  ...
  · studies: ==각 실험을 하나씩, {title(논문 표현), purpose(목적), setup(실험 세팅: 데이터·모델·비교군·조건), result(결과 — 논문이 보고한 실제 수치 포함 + 의미)}로==. ablation·분석 실험도 하나의 study로.
