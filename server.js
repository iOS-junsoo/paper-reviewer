  ...
  "equations": [
    {
      ...
      "anchor_above": "paper_page에서 이 수식 '바로 위' 줄의 본문 텍스트를 그대로 짧게 인용(3~8단어). 수식 자체가 아니라 주변 본문 문장이어야 하고, PDF에서 정확히 찾을 수 있게 원문 그대로. 위 줄이 없으면 null",
      "anchor_below": "paper_page에서 이 수식 '바로 아래' 줄의 본문 텍스트를 그대로 짧게 인용(3~8단어). 없으면 null",
      "bbox": { "x0": 0.12, "y0": 0.34, "x1": 0.88, "y1": 0.40, "설명": "(보조) anchor를 못 찾을 때만 쓰는 대략 위치. 정규화 좌표(좌상단 0,0~우하단 1,1). 앵커가 있으면 생략 가능" },
...
- equations: 논문의 핵심 수식만 3~8개. ==배열 순서는 계산이 흘러가는 순서(앞 수식의 출력이 뒤 수식의 입력이 되는 순서)로 정렬하세요==. 순서를 재배열하더라도 paper_ref에 원 논문의 수식 번호(Eq. N)나 절 번호를 남겨 사용자가 원문과 대조할 수 있게 하세요. variables에는 수식에 등장하는 주요 기호를 하나도 빠짐없이 나열하고, meaning은 비전공자도 이해할 만큼 쉬운 말로 ("~에 해당", "~를 뜻함" 같은 직관적 설명). explanation은 수식의 역할과 방법론 단계 연결, analogy는 설명 바로 아래에 표시될 일상 비유 한 문장. ==anchor_above/anchor_below에는 그 수식 바로 위·아래 줄의 '본문 텍스트'를 원문 그대로 짧게 인용하세요== (수식·기호가 아니라 일반 문장. 예: 위="We compute the matrix of outputs as", 아래="The two most commonly used"). 프론트가 이 문장을 PDF에서 찾아 그 사이(=수식 영역)에 정확히 하이라이트합니다 — 그러니 ==반드시 PDF에 그대로 있는 문구를 정확히== 옮기세요(의역·축약 금지). 위/아래 줄이 없으면 그쪽만 null. 둘 다 어려우면 bbox로 대략 위치만. 수식이 없는 논문이면 빈 배열 [].
