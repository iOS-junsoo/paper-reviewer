...
//
// ==상한은 '예산'이 아니라 '천장'이다== — 올려둔다고 응답이 길어지거나 비용이 늘지 않고,
// 실제 생성한 토큰만큼만 과금된다. 따라서 내용을 깎아 상한에 맞출 이유가 없다.
// 모델 최대치인 128K로 둬서 긴 논문(그림·수식이 많은 경우)도 잘리지 않게 한다.
// 실측: NeuronFair 13p 정밀 분석이 54,736 토큰 — 64K였다면 여유가 14%뿐이었다.
...
6. ==완성도 자가점검 (출력 직전 반드시 수행)==: JSON을 끝내기 전에, 본문을 처음부터 다시 훑어 아래를 빠뜨리지 않았는지 확인하세요.
   ...
   항목 수도, 각 항목의 설명 깊이도 줄이지 마세요. 출력 길이는 충분히 허용돼 있습니다.`;
