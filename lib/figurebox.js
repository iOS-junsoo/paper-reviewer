...
const OTHER_CAP_RE = /^(table|figure|fig\.?)\s*\d+\s*[:.]/i; // 다른 그림/표 캡션 줄
...
async function analyzePage(pdfPath, page, parsed, modelBox, wpt, hpt) {
  ...
  if (parsed) {
    ...
    const normType = (t) => {
      ...
      return n === "fig" ? "figure" : n === "tab" || n === "tbl" ? "table" : n;
    };
    ...
    if (cands.length) {
      ...
      if (cands[0].score >= 3) cap = cands[0]; // 확신 없으면 캡션 없음 취급(원본 bbox 폴백과 동일)
    }
  }
  ...
  return { words, lineH, rows, ci, cap, capBlock, foreignBlocks, colWords, colMin, colMax, modelBox, wpt, hpt };
}
