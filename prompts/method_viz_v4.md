# 연구 방법론 시각화 생성 지시문 v4.2 (판정 → 라우팅 → 생성)
...
```jsonc
{
  "method_visualization": {
    ...
    "control": {            // 정확히 1개
      ...
      "evidence": "direction·범위 판정의 근거가 된 논문 원문 내용 요약 한 문장 (mode=mask면 필수). 렌더러는 미사용 — 검수자(사람/감사 모델)가 direction 반전을 스펙만 보고 잡기 위한 감사 필드. 예: \"§4.1에서 α를 'the ratio of pruned heads'로 정의 → 클수록 제거 → remove_top\""
    },
...
### §3-2. modules[].expand — 모듈 내부 파이프라인 (선택)

방법론 이해에 내부 구조가 필수인 모듈은 `expand`로 내부 파이프라인을 중첩할 수 있다.
렌더러는 이를 "클릭하면 해당 모듈 아래에 펼쳐지는 내부 파이프라인"으로 그린다.
...
"modules": [{
  ...
  "primitive": "iso_stack",
  "expand": {
    "source_ref": "Fig.1 좌측 블록",   // 필수 — 내부 구조를 확인한 논문 위치
    "modules": [                       // 2~6개. 프리미티브 사전(§4) 동일
      { "id": "enc.mha", "name": "Multi-Head Attention", "primitive": "op_box" },
      { "id": "enc.norm1", "name": "Add & Norm", "primitive": "op_box" }
    ],
    "edges": [ { "from": "enc.mha", "to": "enc.norm1", "kind": "forward" } ],
    "steps": [                         // 선택. 상위 스텝 뒤에 3.1, 3.2 형식으로 삽입됨
      { "module": "enc.mha", "title": "...", "desc": "...", "detail_viz": { } }
    ]
  }
}]
```
...
규칙:
- **깊이 최대 2** (expand 안의 모듈이 한 번 더 expand 가능, 그 이상 금지).
- **id는 `부모id.자식id`로 전역 고유**. steps·edges·control.affects 모두 이 전역
  id로 참조한다.
- expand 내부에도 §4 프리미티브 반복 제한, §5 detail_viz 사전·binds 규칙,
  §7-4 글자수 상한이 동일하게 적용된다. detail_viz 반복 금지는 **레벨 내에서**
  적용한다 (레벨 0에서 쓴 type을 내부에서 다시 쓰는 것은 허용).
- 수식 표기는 깊은 레벨에서 허용·권장된다: 내부 모듈의 name/sub에 짧은 수식
  (`QKᵀ/√d_k`)을 쓸 수 있다. 단 글자수 상한은 동일.
...
- (b) 골격: `io_cube → iso_stack(제안 구조를 스테이지 단위로 축약) → op_box(head) → 출력`.
  ...
  반복 유닛(인코더 블록 등)은 레벨 0에서 iso_stack + loop_badge로 압축하되, **그 모듈의
  expand에 유닛 내부 구성(depth 1)을 반드시 채운다** (T1에서 내부 구성은 논문 그림에
  항상 있으므로 §7-7-1을 충족한다). 제안의 핵심 연산 1개는 depth 2로. 예: Transformer
  레벨 0 = 6모듈, 인코더 expand = MHA·Add&Norm·FFN·Add&Norm, MHA expand = QKV 투영 →
  스케일드 닷 → softmax 가중합 → concat → 출력 투영.
...
- (f) 함정: 층 전부 그리기(과밀), 실제 채널 수를 슬래브 수로 옮기기, control을 억지로
  mask로 지정하기. 디테일을 레벨 0에 욱여넣어 모듈 수를 늘리는 것 — 디테일의 자리는 expand다.

### T2. 모듈·블록 제안
- (a) 주인공: 제안 블록의 **내부** 연산 순서. 전체 망은 무대일 뿐이다.
- (b) 골격: 레벨 0 = 백본 `iso_stack` + 제안 블록이 꽂히는 위치의 `op_box`(강조), 그
  op_box의 **expand(depth 1)에 블록 내부 연산을 채운다**. 블록이 백본 어디에 꽂히는지
  iso_stack에서 그 op_box로 들어가는 edge로 표시한다. `groups`는 이제 "동시에 보여야
  하는 병렬 영역"(예: 논문 그림의 점선 영역) 전용으로만 쓴다.
...
- (d) detail_viz: 블록 내부 단계들에 histogram/transform을 배정하고, 블록 밖(백본)
  단계는 최대 1개로 제한.
- (e) 스테퍼: 백본 진입(1단계) → 제안 블록 단계(여기에 "내부 단계 보기"로 expand.steps
  3~4개 삽입) → 백본 복귀(1단계).
...
**7-6. 출력 직전 자가검증 (필수 절차)** — 스펙을 완성한 뒤 출력하기 전에 아래 7항을
스스로 점검하고, 하나라도 실패하면 해당 부분을 수정한 뒤 다시 점검한다. 점검 결과를
출력에 포함하지는 않는다.

1. **direction 근거**: mode=mask면 evidence에 논문 원문 기반 근거가 있고, 그 근거와
   direction 값이 논리적으로 일치하는가. "클수록 남긴다/제거한다"를 원문에서 확인하지
   못했다면 mask 모드를 포기하고 text로 강등하라 — 방향을 추측으로 정하는 것이 가장
   나쁜 결과다.
2. **라벨 낭독 검사**: 모든 name, name_short, sub, edge label, 그룹 label을 한 번씩
   소리 내어 읽듯 재확인한다. 오탈자("기여"→"기어" 류), 조사 누락, 의미가 통하지 않는
   축약이 없는가.
3. **글자수 상한**: §7-4의 상한(name 한글 10자, sub 8자, edge label 6자, name_short
   5자)을 항목별로 전수 확인. 초과 항목은 지금 줄여라 — 렌더러 말줄임에 맡기지 마라.
4. **관통 예시 전수 등장**: 모든 steps[].desc에 관통 예시의 현재 형태가 언급되는가.
   누락 단계는 desc를 보강하라.
5. **binds 정합**: control이 영향을 주는 모듈의 step에 binds가 "control"을 포함한
   detail_viz가 배정되어 있는가. affects에 있는데 어떤 step에서도 control 바인딩이 없으면
   슬라이더 조작을 관찰할 곳이 없는 스펙이다.
6. **수치 출처**: 스펙 안의 모든 수치(control 범위, 레이어 축약 비율의 원본 수치 병기,
   캡션 값)에 대해 "논문에서 봤다 / 예시값으로 표기했다" 둘 중 하나가 성립하는가. 둘 다
   아니면 그 수치를 제거하라.
7. **금지사항 재확인**: §8의 9개 항목을 순서대로 위반 여부 확인.

## §7-7. expand 작성 규칙 (정직성 우선)

1. **확인한 것만 펼친다**: 논문의 그림·본문·의사코드에서 내부 구조를 직접
   확인한 모듈만 expand를 채우고, `source_ref`에 그 위치를 쓴다. 확인하지
   못한 모듈의 expand는 **비워두는 것이 정답**이다 — 빈 expand는 실패가 아니라
   정직이며, 그럴듯한 내부 구조의 창작은 수치 지어내기(§7-2)와 같은 급의
   실격 사유다. 유명 논문이라 통념상 아는 구조라도 논문 원문 근거를 우선한다.
2. **펼칠 가치가 있는 것만 펼친다**: 방법 이해에 필수인 내부(제안된 블록,
   반복 유닛의 실제 구성)만. 표준 부품의 뻔한 내부(예: 일반 FFN의 두 선형층)는
   sub·data_state 텍스트로 충분하다. 논문당 expand는 1~3개를 권장한다.
3. **관통 예시 유지**: expand.steps의 desc에도 관통 예시의 현재 형태를 쓴다
   ("문장의 각 토큰 벡터가 8개 헤드로 쪼개진다").
4. **깊이 배분**: 레벨 0 = 논문 전체 골격(기존 4~7 모듈 규칙 유지),
   depth 1 = 반복 유닛·제안 블록의 구성, depth 2 = 그 안의 핵심 연산 1개만.
...
8. source_ref 없는 expand, 논문에서 확인하지 않은 내부 구조의 창작 금지.
9. 깊이 3 이상의 expand 금지. expand 내부 id에 `부모id.` 접두 누락 금지.
...
| expand.source_ref 존재 | 해당 expand 제거 |
| expand 깊이 ≤ 2, 내부 modules 2~6개 | 초과 깊이 무시 / 범위 밖이면 해당 expand 제거 |
| expand 내부 id가 `부모id.자식id` 전역 고유 | 해당 expand 제거 |
| expand 내부 steps·edges·detail_viz — 기존 규칙 재귀 적용 | 기존과 동일한 강등 |
