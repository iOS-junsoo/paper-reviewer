...
- `method_steps`는 방법론 텍스트 단계다(수식 탭이 `#step-{i}`로 점프해 오므로 유지). HTML의
  ...
  ==스텝 수 결정 절차(강제): 먼저 방법의 논리 단위(별도 메커니즘·별도 수식·별도 입출력)를
  전부 나열하고, 그 개수를 그대로 스텝 수로 쓴다(대개 4~9). 두 메커니즘을 한 스텝으로
  합치지 마라== — 예: 조건화 주입과 노이즈 예측, 이분 매칭과 손실 계산, coarse/fine
  샘플링은 각각 별도 스텝. 참조 구현의 6스텝은 그 논문의 예시이지 기준이 아니다
  (구조·CSS·패턴만 따르고 스텝 개수는 맞추지 말 것). ==알려진 실패 모드: 무엇이든
  6스텝으로 수렴하는 경향 — 6이 나오면 합쳐진 단위가 없는지 재검토하라.==
